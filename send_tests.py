...
WEBHOOK_BASE = Path("webhook_base.txt").read_text(encoding="utf-8").strip().rstrip("/")
...
def load_phase1_tests() -> List[TestCase]:
    ...
    config = [
        {
            "path": "phase1/html-img-tests.html",
...
def export_manifest(cases: Iterable[TestCase]) -> None:
    ...
    Path("phase1-run-manifest.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"已写入 run manifest: phase1-run-manifest.json（run_id={RUN_ID}）")
